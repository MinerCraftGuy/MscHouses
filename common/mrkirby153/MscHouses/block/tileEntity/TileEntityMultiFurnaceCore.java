...
import mrkirby153.MscHouses.block.BlockMultiFurnaceCore;
...
public class TileEntityMultiFurnaceCore extends TileEntity implements IInventory
{
		...
		for(int horiz = -1; horiz <= 1; horiz++)	// Horizontal (X or Z)
		{
			for(int vert = -1; vert <= 1; vert++)	// Vertical (Y)
			{
				for(int depth = 0; depth <= 2; depth++)	// Depth (Z or X)
				{
					...
					worldObj.setBlock(x, y, z, ModBlocks.multiFurnaceDummy.blockID);
		...
		for(int horiz = -1; horiz <= 1; horiz++)	// Horizontal (X or Z)
		{
			for(int vert = -1; vert <= 1; vert++)	// Vertical (Y)
			{
				for(int depth = 0; depth <= 2; depth++)	// Depth (Z or X)
				{
					...
					if(blockId != ModBlocks.multiFurnaceDummy.blockID)
						continue;
