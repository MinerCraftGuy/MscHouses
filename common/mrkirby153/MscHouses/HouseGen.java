...
public class HouseGen {
	...
	public void hut(int x, int y, int z, World world){
		...
		for(int x1 = 0; x1 < 5; x1++){
			for(int z1 =0; z1 < 5; z1++){
				world.setBlock(x+x1, y-1, z+z1, Block.planks.blockID, 0, 0);
			}
		}
		//Celing
		for(int x1 = 0; x1 < 5; x1++){
			for(int z1 =0; z1 < 5; z1++){
				world.setBlock(x+x1, y+5, z+z1, Block.planks.blockID, 0, 0);
			}
		}
		//North wall
		for(int i = 0; i < 5; i++){
		...
		for(int i = 5; i > 0; --i){
			for(int j = 0; j < 5; j++){
				world.setBlock(x+4, y+j, z+i-1, Block.planks.blockID, 0, 0);
			}
		}
		//South Wall
		for(int i = 5; i > 0; --i){
	...
	public void ninbynineDelux(World world, int x, int y, int z) {
		...
		inventory.setInventorySlotContents(1, new ItemStack(Item.doorWood));
		inventory.setInventorySlotContents(2, new ItemStack(Block.pressurePlateStone));
		inventory.setInventorySlotContents(3, new ItemStack(Block.stoneButton));
		inventory.setInventorySlotContents(9, new ItemStack(Item.axeSteel));
		inventory.setInventorySlotContents(10, new ItemStack(Item.pickaxeSteel));
		inventory.setInventorySlotContents(11, new ItemStack(Item.shovelSteel));
		inventory.setInventorySlotContents(12, new ItemStack(Item.swordSteel));
		inventory.setInventorySlotContents(13, new ItemStack(Item.coal, 64));
		}
		//Put coal in the furnace
		IInventory inventoryFurnace = (IInventory) world.getBlockTileEntity(x+7, y, z+3);
